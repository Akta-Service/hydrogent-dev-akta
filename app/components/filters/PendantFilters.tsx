...
const PendantFilters: React.FC<Props> = ({
  ...
  const getAppliedFilters = () => {
    ...
    // Handle both array and string cases for selectedColor
    ...
    if (selectedStyle) {
      const styleLabel =
        diamondShapeCollections.find((c) => c.handle === selectedStyle)
          ?.title || selectedStyle;
      filters.push({
        type: 'style',
        label: styleLabel,
    ...
    if (allFiltersData.Price) {
      ...
      if (priceData.type === 'numeric') {
        const {minValue, maxValue, config} = priceData;
        const suffix = config?.suffix || '';
        filters.push({
          type: 'price',
          label: `${minValue}${suffix} - ${maxValue}${suffix}`,
          value: 'price',
        });
      }
    }
    ...
    <div className="relative space-y-6 mt-6">
        ...
        <div className="flex flex-wrap items-center gap-3 pb-4">
              ...
              <button
                onClick={() => {
                  if (filter.type === 'color') {
                    // For color filters, pass null to clear the selection
                    handleColorSelect(null);
                  } else if (filter.type === 'style') {
                    // For style filters, pass null to clear
                    handleStyleSelect(null as any);
          ...
          {appliedFilters.length > 1 && onClearAllFilters && (
            <button
              ...
              className="text-[#E7E7E7] hover:text-primary outfit text-sm underline transition-colors"
            >
              Clear All
            </button>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="relative" ref={metalRef}>
          ...
          {openSections.metal && (
            <div className="absolute top-full left-0 right-0 z-50 rounded-[8px] bg-white p-4 border border-[#454545] animate-in slide-in-from-top-2 duration-200">
      ...
      ${selectedColor && selectedColor.includes(handle) ? 'ring-1 ring-black' : ''}`}
                  >
                    <img
        ...
        {/* <div className="relative" ref={styleRef}>
          ...
          >
            <div className="text-left outfit font-light">
              <div className="text-[13px] leading-[15px] text-black">Shape</div>
              <div className="text-black text-[14px] leading-[16px]">
          ...
          {openSections.style && (
            <div className="absolute top-full left-0 right-0 z-50 bg-white rounded-[8px] p-4 border border-[#454545] animate-in slide-in-from-top-2 duration-200">
              <ShopBySection
                title=""
        ...
        <div className="relative" ref={priceRef}>
          ...
          {openSections.price && (
            <div
              className={`absolute top-full left-0 right-0 z-50 bg-white rounded-[8px] p-4 border border-[#454545] animate-in slide-in-from-top-2 duration-200 ${
